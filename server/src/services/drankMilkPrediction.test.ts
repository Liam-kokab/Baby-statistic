...
describe('drankMilkPrediction', () => {
  ...
  describe('divideDataByHours', () => {
    ...
    it('places invalid date records into the first bucket', () => {
      const base = Date.now();
      const bad: TDrankMilk = { id: 1, amount: 30, source: 'FRIDGE', createdAt: 'not-a-date' };
      ...
      const out = divideDataByHours([bad, early, good], 2);
      // bad should be placed in first bucket
      assert.ok(out.length >= 2);
      assert.ok(out[0].some((x) => x.createdAt === bad.createdAt));
