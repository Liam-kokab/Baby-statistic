...
client/
  src/
    main.tsx                        # entry — imports global.css, mounts <App /> inside <BrowserRouter> wrapped in <WsProvider>
    ...
    contexts/
      WsProvider.tsx                 # `<WsProvider>` + `useWsConnected()` — owns the single app-wide WebSocket connection (see "Live Updates" below); mounted once around `<App />` in main.tsx
    components/
      ...
      BlackScreenOverlay/            # BlackScreenOverlay.tsx + .module.css — shared "always on display" overlay (time/sleep/pump/bottle/todayMilk/medicines readout), rendered by both HomePage and PageLayout via useBlackScreen
    ...
    utils/
      ...
      blackScreenPrefs.ts # localStorage-backed HomePage black-screen readout prefs: `BLACK_SCREEN_FIELDS` (`'time' | 'sleep' | 'pump' | 'bottle' | 'todayMilk' | 'medicines'`), getHiddenBlackScreenFields/saveHiddenBlackScreenFields (key `blackScreenHiddenFields`, default: none hidden), getBlackScreenOpacityPercent/saveBlackScreenOpacityPercent (key `blackScreenOpacity`, whole percent 0–100, default `15`) — the readout text is always white, this opacity is what keeps it dim
      medicineStatus.ts # `isMedicineTakenToday(medicine)` — whether a medicine's most recent dose was logged today (local calendar date); shared by `HomePage`'s medicines widget and `BlackScreenOverlay`'s `medicines` readout field
      ...
      resourceKeys.ts    # `TResource` — coarse-grained resource key union (`servedMilk`/`drankMilk`/`sleep`/`nappy`/`medicine`/`pumping`/`milestone`), mirrors the server's `TResource` (server/src/ws/eventBus.ts); `pee`/`poop` share `nappy`; also exports `getResourceForUrl(url)` (mirrors the server's `ROUTE_TO_RESOURCE`), used by `authFetch.ts`
      resourceCache.ts   # module-level cache shared across every page mount for the tab's lifetime: `getResourceEntry`/`subscribeResourceEntry`/`fetchResource`/`registerResourceDependencies` back `useResource`; `markResourceDirty(resource)`/`markAllResourcesDirty()` (called from `WsProvider`) mark matching entries stale; `subscribeResourceKind`/`subscribeAnyResourceChange` back `useResourceListener`; `clearResourceCache()` wipes everything on logout
      useResource.ts     # hook: `(key, fetcher, resources, enabled = true) => { data, isError, loading, lastUpdatedAt, refresh }` — reads/refetches a `resourceCache` entry; shows cached data instantly on remount (no flash) unless marked dirty since the last fetch (a matching WS `update`, or any connection drop); see "Live Updates" below
      useResourceListener.ts # hook: `(resources, onChange) => void` — imperative "refetch now" signal for consumers that don't use the cache directly (e.g. paginated `useTimeWindowScroll` lists), fired on a matching WS `update` or any connection drop, only while mounted
      useAlwaysOnDisplayData.ts # hook: reads GET /api/home/always-on-display via `useResource`, sharing one cache entry/key across every page (`BlackScreenOverlay` is mounted everywhere) — `enabled: active` keeps `useResource`'s own dirty-triggered auto-fetch live while the overlay is open (so a WebSocket update refreshes the readout in real time), *plus* an explicit effect always refetches on every `false → true` transition of `active` regardless of dirty state (so opening it is never stale even if a prior update was somehow missed). Powers BlackScreenOverlay's readout on every page.
      useDataFreshness.ts # `TDataFreshness` — `{ lastUpdatedAt, isError, wsConnected? }`, the shared shape passed to `PageLayout`'s `dataFreshness` prop / rendered directly by `DataFreshnessDot`. Pages get `lastUpdatedAt`/`isError` from `useResource()` and merge in `wsConnected` from `useWsConnected()` (typically via `useLiveDataSync`, which returns the combined object)
...
- `onBlackScreenOpenChange` fires whenever the (internally-owned) black-screen open state changes, so the page can mirror it into its own `isBlackScreenOpen` state and gate its own `useRefetchOnVisible`/`useResourceListener`-driven refetches via `enabled`/an `if (!isBlackScreenOpen)` check (further combined with `!wsConnected` on `useRefetchOnVisible`, see "Live Updates" below) — exactly like `HomePage` does with its own `useBlackScreen`. Every page using `useLiveDataSync` (`PoopPeePage`, `SleepPage`, `PumpingPage`, `MedicinePage`, `MilkSavedPage`, `MilkDrankPage`, `MilestonePage`) does this. The WebSocket connection itself is always shared/global (`contexts/WsProvider.tsx`) regardless of this flag — opening the black screen never opens a second connection.
...
- 🟢 green — the single app-wide WebSocket connection (`contexts/WsProvider.tsx`) is currently open, so data updates in real time (see "Live Updates (WebSocket)" below)
...
Most data pages get `data`/`isError`/`lastUpdatedAt` directly from one or more `useResource()` calls (`utils/useResource.ts`) instead of managing their own `setState`/`reportSuccess`/`reportError` calls. `useLiveDataSync()` (`utils/useLiveDataSync.ts`) merges in `wsConnected` from `useWsConnected()` (`contexts/WsProvider.tsx`) and returns the combined `dataFreshness` object to pass to `PageLayout`. `HomePage` — which has its own custom hero instead of `PageLayout` — renders `DataFreshnessDot` directly with the same shape. Wired up on `HomePage`, `PoopPeePage`, `SleepPage`, `PumpingPage`, `MedicinePage`, `MilkSavedPage`, `MilkDrankPage`, and `MilestonePage`; omitted on pages without genuinely "live" server data (Settings, Login, Admin, White Noise, Edit* single-record forms), where `PageLayout`'s `dataFreshness` prop is simply left unset and no dot renders.

## Live Updates (WebSocket) & Global Data Cache

A single WebSocket connection and a shared, cross-page data cache work together to keep every page's data live without duplicate connections or unnecessary refetches:

### `WsProvider` (`contexts/WsProvider.tsx`)
Mounted **once**, around `<App />` in `main.tsx` — not per-page. This is the key architectural change from the previous per-page-socket design: navigating between pages, or opening/closing the black-screen overlay, no longer tears down and reopens a WebSocket connection, since there's exactly one connection for the whole tab's lifetime.
- Opens `wss://<host>/ws`, then sends `{ "type": "auth", "token": "<accessToken>", "clientId": "<uuid>" }` as its first message (the current access token from `authStore`, plus a random per-tab ID from `utils/wsClientId.ts`) rather than a `?token=` query param, since query strings end up in nginx/proxy access logs and browser devtools/history. The server (`server/src/ws/wsServer.ts`) verifies the token, scopes the connection to that user's `babyId`, and replies `{ "type": "auth-ok" }` once authenticated.
- **Echo suppression** — `utils/wsClientId.ts`'s `getWsClientId()` generates one random UUID per tab/page-load and is sent both in the WS auth message and as the `X-Ws-Client-Id` header on every `authFetch` request. When *this* tab makes a change, the server already knows not to notify the connection with that same `clientId` back. Every *other* connection (other tabs, other devices, even other devices logged in as the same username) is still notified as normal.
- **Visibility-aware pausing** — via the Page Visibility API, the socket is closed as soon as the tab is backgrounded or the screen turns off; no point holding a connection (and its 30s server-side heartbeat) open for a page nobody can see. When the page becomes visible again, everything is marked dirty (see below — nothing was being notified while paused) and the socket reconnects right away, bypassing the exponential backoff delay.
- Auto-reconnects with exponential backoff (1s → 30s cap) on close/error (network drop, server restart, expired token). Exposes `useWsConnected(): boolean`, consumed by `DataFreshnessDot` (green when connected) and `useLiveDataSync`.
- In dev, Vite proxies the `/ws` path with `ws: true` (`client/vite.config.ts`) alongside the existing `/api` proxy.

### Resource cache (`utils/resourceCache.ts`, `utils/useResource.ts`)
Each `update` WebSocket message now carries a `resource` field (`TResource` — `servedMilk`/`drankMilk`/`sleep`/`nappy`/`medicine`/`pumping`/`milestone`; `pee`/`poop` share `nappy`), derived server-side from the mutating route's path (see `doc/server.md`). `WsProvider` doesn't fetch anything itself — it just tells `resourceCache.ts` what to invalidate:
- **On a matching `update`** — `markResourceDirty(resource)` marks only the cached entries that depend on that resource as dirty.
- **On this tab's own successful mutation** — `authFetch.ts` also calls `markResourceDirty(resource)` directly (via `utils/resourceKeys.ts`'s `getResourceForUrl`, mirroring the server's route→resource mapping), for any `POST`/`PUT`/`PATCH`/`DELETE` request. This is needed *in addition to* the WebSocket broadcast: the server deliberately skips echoing the `update` back to the exact tab/connection that caused it (see "Echo suppression" below), which was safe under the old per-page-refetch design but would otherwise leave *other* pages' already-cached data for the same resource stale within the same tab — e.g. toggling sleep on `HomePage`, then opening `SleepPage` (already visited earlier this session) would show a stale stats bar without this.
- **On any disconnect — even a momentary one** — `markAllResourcesDirty()` conservatively marks *every* cached entry dirty, since an update could have been missed while offline and there's no way to know which resource(s) it touched.
- **On logout** — `clearResourceCache()` (called from `NavBar`'s logout handler and both failure paths in `authFetch.ts`'s token-refresh logic) wipes the entire cache, so no data lingers into the next login. A full page reload (app closed/reopened) achieves the same thing implicitly, since the in-memory cache doesn't survive a reload anyway.

`useResource(key, fetcher, resources, enabled = true)` is how pages read cached data: it fetches once via `fetcher` on first use, shows the cached value instantly on every later mount (no loading flash) as long as nothing marked it dirty in the meantime, and refetches automatically the moment it's marked dirty — whether that happens while the component is mounted (immediate reactive refetch) or while it was unmounted (checked again on the next mount). This directly satisfies "data is good as long as there's no disconnect, unless we get an update" — the dirty flag *is* that contract, persisted at the module level so it survives page navigation. Pages that don't need a full cached value (e.g. the paginated `useTimeWindowScroll` lists, which already refetch their current window on every mount) use the lighter `useResourceListener(resources, onChange)` instead — just an imperative "refetch now" signal while mounted, with no persisted dirty-tracking of its own.

`useLiveDataSync(resources, onListUpdate, freshness)` (`utils/useLiveDataSync.ts`) is the small wrapper most data pages use to wire all of this into `PageLayout`: it reads `useWsConnected()`, calls `onListUpdate` (typically a paginated list's `refresh()`) via `useResourceListener` whenever a resource changes, falls back to `useRefetchOnVisible`'s polling only while the black screen is open or the WebSocket is disconnected, and returns `{ visibilityRef, dataFreshness, onBlackScreenOpenChange }` for `PageLayout`. `HomePage` wires the equivalent pieces up manually (its own `useBlackScreen` call, its own `useResource`-backed summary) since it doesn't use `PageLayout`.
...
- **Data loading**: `GET /api/home/summary` (`THomeSummary`) combines sleep/drank-milk/suggested-amount/pumping/nappy/medicines into one call, read via a single `useResource` call keyed to that endpoint (resources: `sleep`/`drankMilk`/`pumping`/`nappy`/`medicine`) — cached across mounts, refetched automatically on a matching WebSocket update or any connection drop. After any action (sleep toggle, drank milk, waste, poop/pee, pump, medicine taken) the page calls the same resource's `refresh()` rather than hitting narrower per-resource endpoints. `utils/useRefetchOnVisible` drives the tab-visible/stale-timer refresh fallback — enabled only while the black screen is **closed** and the WebSocket is **disconnected** (`enabled: !isBlackScreenOpen && !wsConnected`) — and resumes with an immediate refetch as soon as either condition changes back (black screen closes, or the WebSocket drops).
...
- **Fullscreen black screen ("always on display")**: tapping/clicking the top hero emoji calls `useBlackScreen().open()`, shared with `PageLayout` (see `BlackScreenOverlay` below) — configured with `keepAwakeMs: BLACK_SCREEN_KEEP_AWAKE_MS` (same on Home and every other page, so it also holds a Screen Wake Lock identically everywhere). Home's own `useRefetchOnVisible` polling fallback is paused for the whole time it's open (`enabled: !isBlackScreenOpen && !wsConnected`), and refetches immediately on exit; the cached `summary` resource (`useResource`) itself keeps reacting to WebSocket updates in the background regardless (harmless — it just keeps the shared cache warm for `BlackScreenOverlay`'s own `GET /api/home/always-on-display` too).
...
- Fetches `GET /api/home/always-on-display` (`TAlwaysOnDisplayData`) via `utils/useAlwaysOnDisplayData.ts` — a `useResource` call sharing one cache entry across every page (since `BlackScreenOverlay` is mounted everywhere), `enabled: active`. Two triggers, both needed: `useResource`'s own dirty-triggered auto-fetch stays live *while the overlay is open* (so a WebSocket update — or a connection drop — that arrives while it's already on screen refreshes the readout in real time), and a separate explicit effect also refetches every time `active` transitions `false → true` (every time it opens) regardless of dirty state, so opening it is never stale even if a prior update was somehow missed. `resourceCache.ts`'s fetch dedup means both can fire on the same open transition without a double-fetch.
- Renders `DataFreshnessDot` in the same top-right spot as every other page's banner (its own CSS positions it absolutely against the overlay, which is `position: fixed`), fed by `useAlwaysOnDisplayData`'s `lastUpdatedAt`/`isError` and `useWsConnected()` — always visible (not tied to the exit-button crossfade below), but dimmed to the same user-configured **Text opacity** as the rest of the readout so it stays unobtrusive.
- While open, any click/tap or mouse movement shows a (`secondary` variant) **Exit** button in the corner, and that button auto-hides after 2 seconds of inactivity — the moment it hides, a dim data readout takes its place; the two never show at once, only crossfading via `opacity`. The readout is split into two positioned groups: the **current time** bottom-right at `2.8rem` (twice the size of the other fields), and **sleeping/awake duration**, **time since last pump**, **last bottle amount + time** (or "no bottle logged"), **today's ml so far vs the last-10-days average** (`{todayMl}/{avgPerDayLast10} ml`, from `drankToday`), and one line per active medicine (`💊 {name} ✅|❌` — whether its most recent dose was logged today, via `utils/medicineStatus.ts` → `isMedicineTakenToday`, shared with `HomePage`'s medicines widget) stacked top-right at `1.4rem`. Each of the six fields is individually toggleable via **Settings → Home → Black Screen** (`Checkmark` per field, `utils/blackScreenPrefs.ts` → `getHiddenBlackScreenFields`/`saveHiddenBlackScreenFields`, localStorage key `blackScreenHiddenFields`). Readout text is always white; a **Text opacity** range slider (0–100%, `getBlackScreenOpacityPercent`/`saveBlackScreenOpacityPercent`, key `blackScreenOpacity`, default `15`) controls how visible it is — the wrapping group elements handle the exit-button crossfade opacity (0/1), while each text `<span>` carries the configured opacity independently, so the two multiply together visually. Every `.blackScreenReadoutText` span also has `filter: grayscale(1)`, so any emoji within it (💊, ✅, ❌) render as unobtrusive grayscale glyphs instead of full color, matching the plain white text. Durations and the clock only ever show hours:minutes (no seconds) and re-render at most once a minute (a dedicated 60s `setInterval`).
...
`MilkDrankPage` shows a stats bar **below** the filter (affected by date range): total ml consumed always, plus a second chip that depends on the active view — in **item view** it's `GET /api/drank-milk/today-stats` (`TDrankMilkTodayStats`) rendered as `{todayMl}/{avgPerDayLast10} ml` (today's ml so far vs the avg ml/day over the 10 days before today, refreshed alongside the rest of the page's live data); in **day/week view** it's the date-range avg ml/day (`total ÷ days in range`) as before. If any entry in the period (or, for the today-stats chip, today/the last 10 days) has `source === 'BOOB'`, the relevant totals/averages are marked with `*` (e.g. `340* ml`) to indicate the figure is likely inaccurate. The same `*` marker appears on day and week group headers when that group contains a BOOB entry. Item cards and expanded day rows show a source emoji (🧊 FRIDGE, ❄️ FREEZER, 🤱 BOOB).  
...
- **🏠 Home** (`SETTINGS_TAB_HOME`): renders `HomeWidgetsEditor` — lets the user show/hide and reorder the HomePage widgets (see `HomePage` docs above) — plus `WhiteNoiseSoundsEditor`, which lets the user pick which sounds (White/Fan/Wave) appear in the HomePage White Noise widget; plus a **Black Screen** card with one `Checkmark` per readout field (time/sleep/pump/bottle/todayMilk/medicines) and a native `<input type="range">` (0–100%, styled via `accent-color`) for the readout's text opacity (see `BlackScreenOverlay` above — these prefs apply to the black screen on every page, not just Home)
