...
- A second `/api` middleware (mounted right after `authenticate`) broadcasts a "this baby's data changed" WebSocket notification (via `ws/eventBus.ts`'s `publishBabyUpdate`) after any mutating request (`POST`/`PUT`/`PATCH`/`DELETE`) that completes with a 2xx status and has a `req.user.babyId` — the mutated route's first path segment (e.g. `served-milk`, `sleep`; `pee`/`poop` both map to `nappy`) is looked up in a `ROUTE_TO_RESOURCE` table and forwarded as the notification's `resource` field so clients can invalidate only the matching cached data — see "Live Updates (WebSocket)" below
...
- **`ws/eventBus.ts`** — `publishBabyUpdate(babyId, resource?, originClientId?)` / `subscribeBabyUpdates(babyId, listener)`, where `listener` receives `(resource?, originClientId?)` so callers can forward the resource and decide whether to skip notifying their own connection. `TResource` (`'servedMilk' | 'drankMilk' | 'sleep' | 'nappy' | 'medicine' | 'pumping' | 'milestone'`) mirrors the client's `TResource` (`client/src/utils/resourceKeys.ts`).
...
- **Connection dedup** — the same `clientId` is also used to keep at most one live connection per tab (`connectionsByClientId` map, keyed by `clientId`). Since the client now holds exactly one app-wide WebSocket connection for its whole lifetime (`client/src/contexts/WsProvider.tsx`, not opened/closed per page anymore), reconnects only happen on real network drops/tab backgrounding rather than on every page navigation. If a tab's socket reconnects (e.g. coming back from being backgrounded) before its previous connection's close frame ever reached the server — common on mobile browsers, which can suspend a hidden tab's outgoing network activity before it gets a chance to send one — the new connection immediately `terminate()`s the stale one on successful auth, instead of waiting for the next heartbeat cycle (up to `HEARTBEAT_MS * 2` = 60s) to notice it's dead.
- Messages sent to clients carry `{ "type": "update", "resource": "<TResource>" }` — no other data — the client uses `resource` to invalidate only the matching cached entries (`client/src/utils/resourceCache.ts`) and is expected to re-run its own existing fetch/refetch logic on receipt; see `doc/client.md`. This keeps the server-side change minimal and avoids duplicating each resource's serialization logic over the socket.
