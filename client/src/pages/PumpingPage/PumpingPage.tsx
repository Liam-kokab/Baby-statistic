...
const RESOURCES: TResource[] = ['pumping'];
...
const PumpingPage = () => {
  ...
  const summaryKey = `/api/pumping/summary?from=${from}&to=${to}`;
  const fetchSummary = useCallback(async () => {
    const params = new URLSearchParams({ from: `${from}T00:00:00`, to: `${to}T23:59:59` });
    return authFetch<TPumpingSummary>(`/api/pumping/summary?${params}`);
  }, [from, to]);
  ...
  return (
    <PageLayout title={t('PUMPING_PAGE_TITLE')} emoji="🥛" gradient="indigo" bannerSlug="pumping" ref={visibilityRef} dataFreshness={dataFreshness} onBlackScreenOpenChange={onBlackScreenOpenChange}>
