...
// A log entry augmented with medicine name for display
...
const MedicinePage = () => {
  ...
  // Groups items by medicine name, preserving insertion order of first occurrence
  ...
  const renderMedItems = (items: TLogWithName[], medKey: string) => {
          ...
          sorted.map((item) => (
            <div key={item.id} className={styles.dayItem}>
              ...
              <Button
                ...
                onClick={() => navigate(`/medicine/log/${item.id}`)}
              />
            </div>
          ))
        ) : null}
  ...
  const handleAddMedicine = async (): Promise<void> => {
    ...
    setAddLoading(true);
    setAddError(null);
    const res = await fetch2('/api/medicine', {
      method: 'POST',
    ...
    return (
      <div className={styles.list}>
        {groups.length === 0 ? (
          ...
          groups.map(({ name, items }) => renderMedItems(items, `item-${name}`))
        )}
      </div>
    );
  };
          ...
          days.map(({ date, items }) => {
            ...
            return (
              <div key={date} className={styles.dayGroup}>
                ...
                {isOpen ? groupByMedicine(items).map(({ name, items: meds }) =>
                  renderMedItems(meds, `day-${date}-${name}`)
                ) : null}
              </div>
            );
          })
                  ...
                  days.map(({ date, items }) => {
                    ...
                    return (
                      <div key={date} className={styles.dayGroup}>
                        ...
                        {isDayOpen ? groupByMedicine(items).map(({ name, items: meds }) =>
                          renderMedItems(meds, `week-${weekKey}-${date}-${name}`)
                        ) : null}
                      </div>
                    );
                  })
