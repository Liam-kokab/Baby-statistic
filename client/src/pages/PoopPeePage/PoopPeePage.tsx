...
const RESOURCES: TResource[] = ['nappy'];
...
const PoopPeePage = () => {
  ...
  const summaryKey = `/api/nappy/summary?from=${from}&to=${to}`;
  const fetchSummary = useCallback(async () => {
    const params = new URLSearchParams({ from: `${from}T00:00:00`, to: `${to}T23:59:59` });
    return authFetch<TNappySummary>(`/api/nappy/summary?${params}`);
  }, [from, to]);
  ...
  return (
    <PageLayout title={t('POOP_PEE_PAGE_TITLE')} emoji="💩" gradient="amber" bannerSlug="poop-pee" ref={visibilityRef} dataFreshness={dataFreshness} onBlackScreenOpenChange={onBlackScreenOpenChange}>
